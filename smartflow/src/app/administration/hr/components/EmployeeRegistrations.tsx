...
import { CheckCircle, XCircle, Eye, X, User, Calendar, Hash, Building2, Settings, MessageSquare, Clock, Mail, Filter, Search, Shield, AlertCircle } from 'lucide-react';
...
const EmployeeRegistrations: React.FC = () => {
  ...
  const pendingCount = accessRequests?.filter((req: any) => req.status === 'pending').length || 0;
  const approvedCount = accessRequests?.filter((req: any) => req.status === 'approved').length || 0;
  const rejectedCount = accessRequests?.filter((req: any) => req.status === 'rejected').length || 0;
  ...
  async function callHelper() {
    try {
      ...
      setOriginalBackendData(result.original);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          <h2 className="text-2xl lg:text-3xl font-bold text-gray-900">Employee Registrations</h2>
          <p className="text-sm text-gray-600 mt-1">Review and manage employee registration applications</p>
        </div>
      </div>
      ...
      {isFetching ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-500">No registration applications match your current filters.</p>
        </div>
      ) : (
        <>
          {/* Desktop Table View */}
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Add a comment for this application..."
                />
              </div>
            </div>
